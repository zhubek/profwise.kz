...
async function main() {
  log('🚀 Starting Complete Profession Population...', colors.yellow);
  ...
  try {
    ...
    // Get all professions without education data
    log('\n🔍 Finding professions without education data...', colors.cyan);
    ...
    const needsData = allProfessions.filter(p => p.education === null);

    log(`✓ Found ${needsData.length} professions without education data`, colors.green);
    log(`\n📝 Starting population process...`, colors.cyan);
    ...
    for (const profession of needsData) {
      ...
      try {
        ...
        if (professionData.education.specializations) {
          withEducation++;
        } else {
          withoutEducation++;
        }
        ...
        // Progress indicator every 20 professions
        if (updated % 20 === 0) {
          log(`\n⏳ Progress: ${updated}/${needsData.length} professions (${Math.floor((updated/needsData.length)*100)}%)`, colors.cyan);
        }
      } catch (error: any) {
    ...
    log(`    - With education specializations: ${withEducation}`, colors.green);
    log(`    - Without education specializations: ${withoutEducation}`, colors.yellow);
    ...
    log(`\n📊 Final Database Summary:`, colors.cyan);
