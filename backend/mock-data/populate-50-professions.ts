...
async function populate50Professions() {
  try {
    ...
    // Get already populated professions
    const populatedProfessions = await prisma.profession.findMany({
      where: {
        descriptionData: { not: null }
      },
      select: { id: true }
    });
    const populatedIds = new Set(populatedProfessions.map(p => p.id));

    console.log(`📊 Database status:`);
    console.log(`   Total professions in DB: ${dbIds.size}`);
    console.log(`   Already populated: ${populatedIds.size}`);
    console.log(`   Available for population: ${dbIds.size - populatedIds.size}\n`);

    // Filter to only professions that exist in DB and are not yet populated
    const validRecords = data.filter((row: any) =>
      dbIds.has(row.jobid) && !populatedIds.has(row.jobid)
    );

    console.log(`✅ Found ${validRecords.length} professions available for population\n`);

    if (validRecords.length < 50) {
      console.log(`⚠️  Warning: Only ${validRecords.length} professions available (less than 50)`);
    }
    ...
    // Select up to 50 random professions with diverse RIASEC codes
    const targetCount = Math.min(50, validRecords.length);
    const selectedRecords: any[] = [];
    const usedIds = new Set();

    // Try to get different RIASEC codes first
    Object.keys(riasecGroups).forEach(code => {
      if (selectedRecords.length < targetCount) {
        const availableRecords = riasecGroups[code].filter((r: any) => !usedIds.has(r.jobid));
        if (availableRecords.length > 0) {
          const record = availableRecords[Math.floor(Math.random() * availableRecords.length)];
          selectedRecords.push(record);
          usedIds.add(record.jobid);
        }
      }
    });

    // If we need more, add random ones
    while (selectedRecords.length < targetCount) {
      const randomRecord = validRecords[Math.floor(Math.random() * validRecords.length)];
      if (!usedIds.has(randomRecord.jobid)) {
        selectedRecords.push(randomRecord);
        usedIds.add(randomRecord.jobid);
      }
    }
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`\n🎉 Successfully populated ${selectedRecords.length} professions!`);
